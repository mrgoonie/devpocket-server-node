...
function validateTemplate(template: any): template is TemplateDefinition {
  const required = [
    ...
    'dockerImage', 'defaultPort', 'defaultResourcesCpu', 
    'defaultResourcesMemory', 'defaultResourcesStorage', 
    'environmentVariables', 'startupCommands', 'status', 'version'
  ];
  ...
  if (!Object.values(TemplateCategory).includes(template.category)) {
    throw new Error(`Invalid category: ${template.category}. Must be one of: ${Object.values(TemplateCategory).join(', ')}`);
  }

  // Validate status
  if (!Object.values(TemplateStatus).includes(template.status)) {
    throw new Error(`Invalid status: ${template.status}. Must be one of: ${Object.values(TemplateStatus).join(', ')}`);
  }
