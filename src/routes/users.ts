...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const environmentsByStatus = stats.environments.reduce(
      (acc, env) => {
    ...
    const recentActivity = {
      environmentsCreatedLast7Days: stats.environments.filter(env => env.createdAt > last7Days)
        .length,
      environmentsCreatedLast30Days: stats.environments.filter(env => env.createdAt > last30Days)
        .length,
      environmentsActiveLastWeek: stats.environments.filter(
        env => env.lastActivityAt && env.lastActivityAt > last7Days
      ).length,
    };
