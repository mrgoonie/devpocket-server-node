...
describe('CI/CD Pipeline Integration Tests', () => {
  ...
  describe('Kubernetes Deployment Sequence Simulation', () => {
    test('should simulate manifest generation for all environments', async () => {
      const generateScript = path.join(process.cwd(), 'scripts', 'deployment', 'generate-manifests.sh');
      ...
      if (fs.existsSync(generateScript)) {
        for (const env of environments) {
          ...
          try {
            ...
            console.warn(`Could not test manifest generation for ${env}:`, error);
          }
        }
      }
    });
  ...
  describe('Error Handling and Rollback Simulation', () => {
    ...
    test('should simulate rollback script execution', async () => {
      const rollbackScript = path.join(process.cwd(), 'scripts', 'deployment', 'rollback-deployment.sh');
      ...
      if (fs.existsSync(rollbackScript)) {
        ...
        try {
          ...
          console.warn('Could not test rollback script:', error);
        }
      }
    });
    ...
    test('should validate job failure handling', () => {
      ...
      workflows.forEach(workflow => {
        ...
        if (fs.existsSync(workflowPath)) {
          ...
          const failureStep = notifyJob.steps.find((step: any) => 
            step.name === 'Deployment Failed'
          );
    ...
    test('should validate environment promotion flow', () => {
      ...
      promotionFlow.forEach(({ env, branch, prerelease }) => {
        if (env === 'prod') {
          ...
          const branchConfig = releaseConfig.branches.find((b: any) => 
            typeof b === 'object' && b.name === branch
          );
  ...
  describe('Resource Cleanup Simulation', () => {
    test('should simulate cleanup script execution', async () => {
      const cleanupScript = path.join(process.cwd(), 'scripts', 'deployment', 'cleanup-environments.sh');
      ...
      if (fs.existsSync(cleanupScript)) {
        try {
          ...
          console.warn('Could not test cleanup script:', error);
        }
      }
    });
    ...
    test('should validate resource cleanup order', () => {
      ...
      if (fs.existsSync(cleanupScript)) {
        ...
        // Check cleanup order
        expect(content).toContain('RESOURCE_ORDER=("ingress" "service" "deployment" "configmap" "secret" "pvc")');
    ...
    test('should validate resource limits and requests in generated manifests', async () => {
      ...
      if (fs.existsSync(generateScript)) {
        try {
          ...
          console.warn('Could not test resource limits:', error);
        }
      }
    });
  });
